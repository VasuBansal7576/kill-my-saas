  ...
  return (
    <div className="rd-workspace">
      <div className="page-head rd-head">
        <div><p className="eyebrow">Evaluations</p><h1>Review plans</h1><p>Independent rounds, private reviewer queues, weighted results, and final program decisions.</p></div>
      ...
      <div className="rd-summary">
        ...
        <Metric label="Decisions" value={workspace.results.filter((row) => row.decision).length} note="Current program outcomes" />
      </div>
      ...
      {decisionTarget ? <DecisionDialog title={workspace.results.find((row) => row.submissionId === decisionTarget)?.title ?? "Proposal"} currentDecision={workspace.results.find((row) => row.submissionId === decisionTarget)?.decision ?? null} reason={decisionReason} setReason={setDecisionReason} busy={busy} close={() => setDecisionTarget(null)} decide={(outcome) => void decide(decisionTarget, outcome)} /> : null}
    </div>
  );
}
  ...
  return <div className="rd-modal-backdrop"><section className="rd-modal" role="dialog" aria-modal="true" aria-labelledby="plan-title">
    ...
    <div className="rd-round-editor">{props.rounds.map((round, roundIndex) => <fieldset key={round.key}>
      ...
      <div className="rd-scorecard-editor"><strong>Scorecard</strong>{round.scorecard.map((criterion, criterionIndex) => <div key={criterion.key}>
        ...
        <label className="rd-field">Weight <small>Relative influence; for example, Weight 2 counts twice as much as Weight 1.</small><input type="number" min="0" max="100" disabled={criterion.type === "free_text"} value={criterion.weight} onChange={(event) => updateCriterion(roundIndex, criterionIndex, { ...criterion, weight: Number(event.target.value) } as ReviewCriterion)} /></label>
        ...
        {criterion.type === "numeric" ? <div className="rd-criterion-details"><label className="rd-field">Minimum<input type="number" value={criterion.min} onChange={(event) => updateCriterion(roundIndex, criterionIndex, { ...criterion, min: Number(event.target.value) })} /></label><label className="rd-field">Maximum<input type="number" value={criterion.max} onChange={(event) => updateCriterion(roundIndex, criterionIndex, { ...criterion, max: Number(event.target.value) })} /></label></div> : null}
        {criterion.type === "dropdown" ? <div className="rd-criterion-details rd-option-editor"><strong>Dropdown options</strong>{criterion.options.map((option, optionIndex) => <div key={`${criterion.key}-${optionIndex}`}><input aria-label={`Option ${optionIndex + 1} label`} value={option.label} onChange={(event) => updateCriterion(roundIndex, criterionIndex, { ...criterion, options: criterion.options.map((candidate, index) => index === optionIndex ? { ...candidate, label: event.target.value } : candidate) })} /><input aria-label={`Option ${optionIndex + 1} score`} type="number" min="0" max="100" value={option.score} onChange={(event) => updateCriterion(roundIndex, criterionIndex, { ...criterion, options: criterion.options.map((candidate, index) => index === optionIndex ? { ...candidate, score: Number(event.target.value) } : candidate) })} /></div>)}</div> : null}
...
function DecisionDialog(props: { title: string; currentDecision: "accepted" | "rejected" | null; reason: string; setReason(value: string): void; busy: boolean; close(): void; decide(outcome: "accepted" | "rejected"): void }) {
  const [confirmed, setConfirmed] = useState(false);
  const changing = props.currentDecision !== null;
  const blockedAcceptedChange = props.currentDecision === "accepted";
  return <div className="rd-modal-backdrop"><section className="rd-modal rd-decision" role="dialog" aria-modal="true"><div className="rd-panel-head"><div><span className="rd-kicker">{changing ? "Change program decision" : "Final program decision"}</span><h2>{props.title}</h2></div><button className="rd-close" onClick={props.close} aria-label="Close decision dialog">×</button></div>{props.currentDecision ? <p><strong>Current decision: {capitalize(props.currentDecision)}.</strong></p> : null}<p>{blockedAcceptedChange ? "This acceptance already created a linked session and speaker onboarding work. Changing it to rejected also requires withdrawing that work, so it cannot be reversed from this screen." : props.currentDecision === "rejected" ? "Changing this result to accepted will create a linked session, speaker access, onboarding tasks, and a new notification." : "Accept creates a linked session, speaker access, onboarding tasks, and a notification. Reject records the result and queues a notification."}</p>{blockedAcceptedChange ? <footer className="rd-modal-actions"><button className="rd-secondary" onClick={props.close}>Close</button></footer> : <><label className="rd-field">Private decision note<textarea rows={5} value={props.reason} onChange={(event) => props.setReason(event.target.value)} /></label>{changing ? <label className="rd-decision-confirm"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />I understand the current result and the changes listed above.</label> : null}<footer className="rd-modal-actions"><button className="rd-secondary" onClick={props.close}>Cancel</button>{props.currentDecision !== "rejected" ? <button className="rd-secondary danger" disabled={props.busy || props.reason.trim().length < 3 || (changing && !confirmed)} onClick={() => props.decide("rejected")}>Reject</button> : null}<button className="primary-action" disabled={props.busy || props.reason.trim().length < 3 || (changing && !confirmed)} onClick={() => props.decide("accepted")}>{props.currentDecision === "rejected" ? "Change to accepted" : "Accept and create session"}</button></footer></>}</section></div>;
}
