...
export function ReviewerQueuePage() {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      setQueue(next);
      setSelectedId((current) => current ?? next.assignments.find((assignment) => assignment.status !== "submitted" && assignment.status !== "recused")?.assignmentId ?? next.assignments[0]?.assignmentId ?? null);
      setError(null);
  ...
  useEffect(() => { setAnswers(selected?.ownResponse?.answers ?? {}); setNotes(selected?.ownResponse?.notes ?? ""); setRecusalReason(""); }, [selected]);
  ...
  async function save(finalize: boolean) {
    ...
    try {
      ...
      setNotice(finalize ? "Review finalized. Your response is now immutable." : "Draft persisted. You can resume it after reload.");
      await reload();
