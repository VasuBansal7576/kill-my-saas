...
export function ReviewerQueuePage() {
  ...
  async function save(finalize: boolean) {
    ...
    try {
      await reviewsRequest(
        `/api/v1/reviewer/events/${eventSlug}/reviews/${selected.assignmentId}`,
        jsonRequest("PUT", { answers, notes, finalize }),
      );
      setNotice(
        finalize
          ? "Review finalized. Ask an organizer to reopen it if a correction is needed."
          : "Draft saved. You can continue it later.",
      );
      await reload();
    } catch (caught) {
      setError(
  ...
  async function recuse() {
    ...
    try {
      await reviewsRequest(
        `/api/v1/reviewer/events/${eventSlug}/reviews/${selected.assignmentId}/recuse`,
        jsonRequest("POST", { reason: recusalReason }),
      );
  ...
  if (!queue)
    return <p className="rd-loading">{error ?? "Loading reviewer queue…"}</p>;
  ...
  return (
    <div className="rd-reviewer-shell">
      <header className="rd-reviewer-head">
        <div>
          <span className="rd-kicker">
            Reviewer portal · {queue.event.name}
          </span>
          <h1>Your assigned reviews</h1>
          <p>
            Only your assigned proposals and your own response are available
            here.
          </p>
        </div>
        <div className="rd-reviewer-count">
          <strong>
            {complete}/{queue.assignments.length}
          </strong>
      ...
      <div className="rd-reviewer-layout">
        <aside className="rd-queue">
          ...
          {queue.assignments.map((assignment) => (
            ...
            >
              ...
              <span>
                ...
                <small>
                  {assignment.roundName} · {assignment.status.replace("_", " ")}
                </small>
              </span>
              {assignment.ownResponse?.weightedScore !== null &&
              assignment.ownResponse?.weightedScore !== undefined ? (
                <b>{assignment.ownResponse.weightedScore.toFixed(1)}</b>
              ) : null}
            </button>
          ))}
        </aside>
        {selected ? (
          <section className="rd-review-form">
            <div className="rd-panel-head">
              ...
              <span className="rd-badge">
                {selected.blind ? "Blind review" : "Author visible"}
              </span>
            </div>
            {selected.blind ? (
              <div className="rd-blind-note">
                Participant identity is hidden. Other reviewers’ scores and
                comments are never shown.
              </div>
            ) : (
              <p className="rd-participants">
            ...
            <article className="rd-abstract">
              <h3>Proposal abstract</h3>
              <p>{selected.abstract}</p>
            </article>
            <div className="rd-scorecard">
              {selected.scorecard.map((criterion) => (
                <label className="rd-field" key={criterion.key}>
                  {criterion.label}
                  <small>{reviewerCriterionHelp(criterion)}</small>
                  {criterion.type === "numeric" ? (
                    <input
                      aria-label={`${criterion.label}, rating from ${criterion.min} to ${criterion.max}`}
                      type="number"
                      min={criterion.min}
                      max={criterion.max}
                      step="1"
                      disabled={
                        ...
                        selected.status === "recused"
                      }
                      value={
                        typeof answers[criterion.key] === "number"
                          ? String(answers[criterion.key])
                          : ""
                      }
                      onChange={(event) =>
                        setAnswers({
                    ...
                    <select
                      ...
                      value={
                        typeof answers[criterion.key] === "string"
                          ? String(answers[criterion.key])
                          : ""
                      }
                      onChange={(event) =>
              ...
              <label className="rd-field">
                ...
                <small>Visible to organizers, never to other reviewers.</small>
                <textarea
                  rows={4}
                  disabled={
                    selected.status === "submitted" ||
            ...
            {selected.status === "submitted" ? (
              <div className="rd-finalized">
                Finalized{" "}
                {selected.ownResponse?.submittedAt
                  ? formatEventDateTime(
                      selected.ownResponse.submittedAt,
                      queue.event.timezone,
                    )
                  : ""}
                . Ask an organizer to reopen this response if you need to make a
                correction.
              </div>
            ) : selected.status === "recused" ? (
              <div className="rd-finalized">
                You recused from this proposal.
              </div>
            ) : (
              <>
                ...
                <div className="rd-recusal">
                  <label className="rd-field">
                    Conflict or recusal reason
                    <input
                      value={recusalReason}
                  ...
                  <button
                    ...
                    disabled={busy || recusalReason.trim().length < 3}
                    onClick={() => void recuse()}
                  >
                    Declare conflict and recuse
                  </button>
                </div>
              </>
            )}
          ...
          <section className="rd-review-form rd-empty">
