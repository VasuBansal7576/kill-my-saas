...
import { fileRequestActionLabel } from "./presentation";
import styles from "./speaker-operations.module.css";
import type { EmployerApprovalStatus, SpeakerPortal } from "./types";
...
export function SpeakerPortalPage() {
  ...
  useEffect(() => {
    if (!portal || !window.location.hash.startsWith("#session-")) return;
  ...
  async function saveProfile(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setEditing(false); setMessage("Profile changes saved. Organizers and public pages now use this information.");
  ...
  async function updateEmployerApproval(status: EmployerApprovalStatus) {
    try {
      setPortal(await requestJson<SpeakerPortal>(`/api/v1/speaker/events/${eventSlug}/employer-approval`, jsonRequest("PATCH", { status })));
      setMessage("Employer approval state saved for the organizer.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Employer approval could not be saved."); }
  }
  ...
  async function uploadHeadshot(file: File) {
    ...
    try {
      ...
      setMessage("Profile headshot saved for organizer and public program views.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Your profile headshot could not be saved."); }
  ...
  return <div className={styles.portalShell}><header className={styles.portalHeader}><div><p className={styles.eyebrow}>Speaker portal · {portal.event.name}</p><h1>Welcome, {speaker.displayName.split(" ")[0]}</h1><p>Your sessions, tasks, profile, and resources for this event.</p></div><span className={`${styles.status} ${styles[speaker.status]}`}>{speaker.status}</span></header>{message ? <div className={styles.notice} role="status">{message}</div> : null}<section className={styles.approvalCheck}><div><strong>Employer approval</strong><small>Tell the program team whether participation is waiting on employer authorization.</small></div><select aria-label="Employer approval status" value={speaker.employerApprovalStatus} onChange={(event) => void updateEmployerApproval(event.target.value as EmployerApprovalStatus)}><option value="not_required">Not required</option><option value="pending">Pending</option><option value="approved">Approved</option></select></section><section className={styles.portalMetrics}><Metric label="Tasks complete" value={`${speaker.taskProgress.complete}/${speaker.taskProgress.total}`} /><Metric label="Assigned sessions" value={String(speaker.sessionCount)} /><Metric label="Resources" value={String(portal.resources.length)} /></section><div className={styles.portalGrid}><section className={styles.stack}><section className={styles.panel}><div className={styles.sectionHead}><h2>Your tasks</h2><span>{speaker.taskProgress.overdue ? `${speaker.taskProgress.overdue} overdue` : "On track"}</span></div>{speaker.tasks.length ? speaker.tasks.map((task) => <article className={styles.portalTask} key={task.id}><span className={`${styles.checkmark} ${task.status === "complete" ? styles.checked : ""}`}>{task.status === "complete" ? "✓" : "○"}</span><div><strong>{task.title}</strong><small>{formatDate(task.dueAt)} · {task.required ? "Required" : "Optional"}</small>{task.description ? <p>{task.description}</p> : null}{task.kind === "form" && task.status === "pending" ? <TaskFormFields configuration={task.configuration} values={responses[task.id] ?? {}} onChange={(key, value) => setResponses((current) => ({ ...current, [task.id]: { ...(current[task.id] ?? {}), [key]: value } }))} /> : null}</div>{task.status === "pending" && task.kind !== "file_request" ? <button className={styles.secondaryButton} type="button" onClick={() => void completeTask(task.id)}>Mark complete</button> : task.kind === "file_request" ? <Link className={styles.secondaryButton} to={`/speaker/events/${eventSlug}/files`}>{fileRequestActionLabel(task.status)}</Link> : null}</article>) : <p className={styles.empty}>You have no assigned tasks.</p>}</section><section className={styles.panel}><div className={styles.sectionHead}><h2>Your sessions</h2><span>Accepted program sessions</span></div>{speaker.assignedSessions.length ? speaker.assignedSessions.map((session) => <article id={`session-${session.id}`} className={styles.sessionCard} key={session.id}><strong>{session.title}</strong><p>{session.abstract}</p><small>{session.contentStatus.replace("_", " ")} · {session.role}</small></article>) : <p className={styles.empty}>No accepted session has been linked yet.</p>}</section></section><aside className={styles.stack}><section className={styles.panel}><div className={styles.sectionHead}><h2>Your profile</h2><button className={styles.textButton} type="button" onClick={() => setEditing((value) => !value)}>{editing ? "Cancel" : "Edit"}</button></div>{editing ? <form className={styles.stack} onSubmit={(event) => void saveProfile(event)}><label>Name<input name="displayName" defaultValue={speaker.displayName} /></label><label>Job title<input name="jobTitle" defaultValue={speaker.jobTitle} /></label><label>Company<input name="company" defaultValue={speaker.company} /></label><label>Biography<textarea rows={6} name="biography" defaultValue={speaker.biography} /></label><label>LinkedIn<input name="linkedin" defaultValue={speaker.socialLinks.linkedin} /></label><label>Twitter / X<input name="twitter" defaultValue={speaker.socialLinks.twitter} /></label><label>Profile headshot<input type="file" accept="image/png,image/jpeg,image/webp" disabled={uploadingHeadshot} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadHeadshot(file); }} /></label><button className={styles.primaryButton}>Save profile</button></form> : <div className={styles.profileSummary}>{speaker.headshotFileId ? <img className={styles.profileImage} src={`/api/v1/speaker/events/${eventSlug}/profile/headshot?v=${speaker.headshotFileId}`} alt={`Headshot of ${speaker.displayName}`} /> : <span className={styles.largeAvatar}>{speaker.displayName.split(/\s+/).map((part) => part[0]).join("").slice(0, 2)}</span>}<strong>{speaker.displayName}</strong><small>{[speaker.jobTitle, speaker.company].filter(Boolean).join(" · ")}</small><p>{speaker.biography || "Add your biography so organizers and attendees know you."}</p></div>}</section><section className={styles.panel}><div className={styles.sectionHead}><h2>Resources</h2><span>Guides and tools</span></div>{selectedResource ? <><button className={styles.textButton} type="button" onClick={() => setActiveResource(null)}>← All resources</button><div className={styles.renderedHtml} dangerouslySetInnerHTML={{ __html: selectedResource.contentHtml }} /></> : portal.resources.length ? portal.resources.map((resource) => <button className={styles.resourceLink} type="button" key={resource.id} onClick={() => setActiveResource(resource.id)}><strong>{resource.title}</strong><small>{resource.summary}</small></button>) : <p className={styles.empty}>No published resources yet.</p>}</section></aside></div></div>;
}
