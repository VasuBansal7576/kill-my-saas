...
import type { RosterSpeaker, SpeakerDetail, SpeakerStatus } from "./types";
...
export function SpeakersPage() {
  ...
  async function openSpeaker(id: string) {
    try {
      setSelected(await requestJson<SpeakerDetail>(`/api/v1/organizer/events/${eventSlug}/speakers/${id}`));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "The speaker could not be loaded.");
    }
  }
  ...
  return (
    <div className={styles.workspace}>
      ...
      {showAdd ? <AddSpeakerForm eventSlug={eventSlug} onSaved={async (speaker) => { setShowAdd(false); setSelected(speaker); await loadRoster(); }} /> : null}
      ...
      <section className={styles.panel}>
        ...
        <div className={styles.table} role="table" aria-label="Speaker roster">
          ...
          {loading ? <div className={styles.empty}>Loading speakers…</div> : speakers.length === 0 ? <div className={styles.empty}>No speakers match these filters.</div> : speakers.map((speaker) => (
            <button className={styles.tableRow} type="button" role="row" key={speaker.eventSpeakerId} onClick={() => void openSpeaker(speaker.eventSpeakerId)}>
              ...
              <span>{speaker.sessionCount}</span><span aria-hidden="true">›</span>
            </button>
          ))}
        </div>
      </section>
      {selected ? <SpeakerDetailPanel eventSlug={eventSlug} speaker={selected} onClose={() => setSelected(null)} onSaved={async (speaker) => { setSelected(speaker); await loadRoster(); }} /> : null}
    </div>
  );
}
