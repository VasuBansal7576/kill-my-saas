...
function OutreachDialog({ organizationId, contacts, events, busy, onClose, onRun, onQueued }: { organizationId: string; contacts: CrmContact[]; events: CrmEvent[]; busy: boolean; onClose: () => void; onRun: (action: () => Promise<void>, fallback: string) => Promise<void>; onQueued: (count: number) => Promise<void> }) {
  ...
  return <AccessibleDialog close={onClose} titleId="crm-outreach-title" backdropClassName={styles.backdrop} dialogClassName={styles.outreach} initialFocus="#crm-outreach-event"><form onSubmit={(event) => void submit(event)}><header><div><span>New message</span><h2 id="crm-outreach-title">Email {contacts.length} selected contacts</h2></div><button type="button" aria-label="Close email composer" onClick={onClose}>×</button></header><div className={styles.outreachGrid}><section><label>Target event<select id="crm-outreach-event" required value={eventId} onChange={(event) => setEventId(event.target.value)}><option value="">Choose event…</option>{events.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Message name<input value={message.name} onChange={(event) => setMessage({ ...message, name: event.target.value })} /></label><label>Subject<input value={message.subjectTemplate} onChange={(event) => setMessage({ ...message, subjectTemplate: event.target.value })} /></label><label>HTML body<textarea rows={7} value={message.htmlTemplate} onChange={(event) => setMessage({ ...message, htmlTemplate: event.target.value })} /></label><label>Plain text<textarea rows={5} value={message.textTemplate} onChange={(event) => setMessage({ ...message, textTemplate: event.target.value })} /></label></section><aside><span>Personalized preview</span><strong>{preview ? personalize(message.subjectTemplate, preview) : "No recipient"}</strong><div>{preview ? personalize(message.htmlTemplate, preview).replace(/<[^>]+>/g, " ") : ""}</div><small>The CRM persists the exact selected audience, target event, and message request. Communications owns rendering, provider delivery, and outcome logs.</small></aside></div><footer><button type="button" onClick={onClose}>Cancel</button><button className={styles.primary} disabled={busy || !contacts.length || !eventId}>{busy ? "Persisting…" : "Create email handoff"}</button></footer></form></AccessibleDialog>;
}
