...
export function SubmissionsPage() {
  ...
  return (
    <div className="cfp-workspace">
      ...
      {decisionTarget ? <AccessibleDialog close={() => { setDecisionTarget(null); setDecisionReason(""); }} titleId="submission-decision-title" backdropClassName="submission-decision-backdrop" dialogClassName="submission-decision-dialog" initialFocus="#submission-decision-note"><p className="eyebrow">Private program decision</p><h2 id="submission-decision-title">{decisionTarget.title}</h2><p>Accept creates the linked Session as a private organizer handoff. Neither outcome, onboarding, nor email is released until you review the staged communication.</p><label>Private decision note<textarea id="submission-decision-note" rows={5} value={decisionReason} onChange={(event) => setDecisionReason(event.target.value)} placeholder="Why is this the right program decision?" /></label><footer><button type="button" className="secondary-action" onClick={() => { setDecisionTarget(null); setDecisionReason(""); }}>Cancel</button><button type="button" className="secondary-action danger" disabled={busyId === decisionTarget.id || decisionReason.trim().length < 3} onClick={() => void decide("rejected")}>Record rejection privately</button><button type="button" className="primary-action" disabled={busyId === decisionTarget.id || decisionReason.trim().length < 3} onClick={() => void decide("accepted")}>{busyId === decisionTarget.id ? "Recording…" : "Accept privately & create Session"}</button></footer></AccessibleDialog> : null}
      {notificationTarget?.decisionNotification ? <AccessibleDialog close={() => setNotificationTarget(null)} titleId="decision-release-title" backdropClassName="submission-decision-backdrop" dialogClassName="submission-decision-dialog decision-notification-dialog" initialFocus="#decision-release-subject"><p className="eyebrow">Decision release queue</p><h2 id="decision-release-title">{notificationTarget.title}</h2><p>The outcome is still private. Review the exact email snapshot, save it, then release the Decision and queue delivery.</p><label>Subject<input id="decision-release-subject" value={notificationSubject} onChange={(event) => setNotificationSubject(event.target.value)} /></label><label>HTML body<textarea rows={7} value={notificationHtml} onChange={(event) => setNotificationHtml(event.target.value)} /></label><label>Plain-text body<textarea rows={5} value={notificationText} onChange={(event) => setNotificationText(event.target.value)} /></label><footer><button type="button" className="secondary-action" onClick={() => setNotificationTarget(null)}>Close</button><button type="button" className="secondary-action" disabled={busyId === notificationTarget.id} onClick={() => void saveNotification()}>Save reviewed message</button><button type="button" className="primary-action" disabled={busyId === notificationTarget.id || notificationStatus !== "reviewed"} onClick={() => void releaseNotification()}>Release & queue notification</button></footer></AccessibleDialog> : null}
      {changeDecisionTarget?.decision ? <AccessibleDialog close={() => setChangeDecisionTarget(null)} titleId="change-decision-title" backdropClassName="submission-decision-backdrop" dialogClassName="submission-decision-dialog" initialFocus="#change-decision-note"><p className="eyebrow">Audited Decision change</p><h2 id="change-decision-title">{changeDecisionTarget.title}</h2><p>{changeDecisionTarget.decision === "accepted" ? "Accepted outcomes have a linked Session and may have placements or a live Publication. The server will reject a reversal that would silently corrupt those downstream records." : "Changing Rejected to Accepted creates the canonical linked Session, resets release state, and stages a fresh acceptance communication."}</p><label>New decision note<textarea id="change-decision-note" rows={4} value={decisionReason} onChange={(event) => setDecisionReason(event.target.value)} /></label><label>Why is the final Decision changing?<textarea rows={4} value={changeReason} onChange={(event) => setChangeReason(event.target.value)} /></label><footer><button type="button" className="secondary-action" onClick={() => setChangeDecisionTarget(null)}>Cancel</button><button type="button" className={changeDecisionTarget.decision === "accepted" ? "secondary-action danger" : "primary-action"} disabled={busyId === changeDecisionTarget.id || decisionReason.trim().length < 1 || changeReason.trim().length < 3} onClick={() => void changeDecision()}>Change to {changeDecisionTarget.decision === "accepted" ? "Rejected" : "Accepted"}</button></footer></AccessibleDialog> : null}
    </div>
  );
}
