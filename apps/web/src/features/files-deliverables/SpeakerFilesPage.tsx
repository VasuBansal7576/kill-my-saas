...
import { hiddenSessionFileCount } from "../speaker-operations/presentation";
...
export function SpeakerFilesPage() {
  ...
  const projectionGap = portal ? hiddenSessionFileCount(portal.speaker.assignedSessions, deliverables) : 0;

  return <div className={styles.portal}>
    <PageHeader headingRef={headingRef} />
    {message ? <div className={styles.notice} role="status">{message}</div> : null}
    {projectionGap > 0 ? <div className={styles.projectionWarning} role="status"><strong>Program handoff pending</strong><p>{projectionGap === 1 ? "This workspace contains a file request tied to a session" : `This workspace contains file requests tied to ${projectionGap} sessions`} that {projectionGap === 1 ? "is" : "are"} not released in Sessions yet. The file request is preserved, but it does not prove an accepted or released session. Ask the organizer to reconcile the program handoff.</p></div> : null}
