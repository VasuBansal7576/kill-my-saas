...
export async function queueDueTaskReminders(database: Database, input: {
  ...
  const dueBefore = input.dueBefore.toISOString();
  const rows = await database.select({
    personId: eventSpeakers.personId,
    taskTitle: speakerTasks.title,
    dueAt: sql<Date>`coalesce(${speakerTaskAssignments.dueAtOverride}, ${speakerTasks.dueAt})`.mapWith(speakerTasks.dueAt),
    ...
    .where(and(
      ...
      sql`coalesce(${speakerTaskAssignments.dueAtOverride}, ${speakerTasks.dueAt}) < ${dueBefore}::timestamptz`,
