...
For `disposable_neon_branch`, reset additionally requires the database to contain exactly the confirmed evaluation organization, then uses a database-native truncate cascade for organization-owned rows. This avoids nondeterministic delete ordering across intentionally restrictive operational links (for example, placements → rooms and calendar recipients → calendar artifacts) while preserving the canonical people/identity tables. `run_scoped` never truncates neighboring runs and keeps the targeted deletion path.
