...
export const speakerTasks = pgTable("speaker_tasks", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("speaker_tasks_event_idempotency_unique").on(table.eventId, table.idempotencyKey)]);
...
export const speakerTaskAssignments = pgTable("speaker_task_assignments", {
  ...
  dueAtOverride: timestamp("due_at_override", { withTimezone: true }),
...
export const speakerResources = pgTable("speaker_resources", {
  id: uuid("id").primaryKey().defaultRandom(),
  eventId: uuid("event_id").notNull().references(() => events.id, { onDelete: "cascade" }),
  ...
  status: speakerResourceStatus("status").notNull().default("draft"),
  visibleToStatuses: jsonb("visible_to_statuses").$type<Array<"invited" | "onboarding" | "ready">>().notNull().default(["invited", "onboarding", "ready"]),
  allowedEmbedOrigins: jsonb("allowed_embed_origins").$type<string[]>().notNull().default([]),
  revision: integer("revision").notNull().default(1),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
  uniqueIndex("speaker_resources_event_slug_unique").on(table.eventId, table.slug),
  index("speaker_resources_event_status_idx").on(table.eventId, table.status),
]);
